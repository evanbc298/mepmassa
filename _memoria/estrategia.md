# Estratégia

> O que importa agora. Prioridades, metas, prazos.
> Atualize sempre que as prioridades mudarem.

## Fase atual — Fase 2 de 5 (Junho 2026)

Fundação de marca concluída. Presença digital e conteúdo em andamento.

## Prioridade principal

Crescimento estruturado em 6 meses (junho a dezembro de 2026): construir autoridade no setor de construção civil, gerar leads qualificados de construtoras e incorporadoras via Instagram orgânico, Meta Ads e Google Ads.

## O que pode esperar

Branding aprofundado e identidade visual completa — trabalhar com o que tiver disponível agora e refinar ao longo do caminho.

## Contexto com prazo

- **Deadline:** Dezembro de 2026 — ter uma operação de marketing rodando de forma consistente e gerando leads
- **Canais ativos:** Instagram, Meta Ads, Google Ads
- **Métrica central:** Volume e qualidade de leads do setor de construção civil

---

## Status das entregas — Junho 2026

### Concluído
- Identidade visual: paleta Navy #1A2B4D + Laranja #F28C28, tipografia Barlow
- Rótulos: barrica 25kg, saco aplicador 3kg v1, apresentação de rótulos
- Site publicado em www.mep.ind.br (MEP Massas + MEP COLOR + blog + calculadora + formulário)
- Blog: 11 artigos SEO publicados
- Google Meu Negócio configurado + automação de posts ativa (Make.com + Gemini AI → publica post toda segunda às 9h com 1 clique)
- Pixel Meta instalado no site
- Email marketing Brevo: domínio autenticado, 5 templates, automação ativa (3 emails / 14 dias)
- Prospecção: 100 construtoras e incorporadoras mapeadas (SC/PR)
- Primeiro carrossel Instagram produzido e publicado: "Como Aplicar Massa Polimérica em 4 Passos" (HTML + Playwright, fotos reais)
- Vídeo depoimento produzido e postado como Reels: "Pedreiro Responde: MEP" (49s, vertical, já editado com legendas)
- Banco de imagens atualizado: fotos antes/depois de área gourmet com MEP COLOR, fotos de obras em andamento (prédio 3 andares, galpão)

### Em andamento
- Produção de carrosseis Instagram — próximo: antes/depois área gourmet (MEP COLOR) e comparativo MEP vs argamassa convencional
- Rótulos MEP COLOR para gráfica — frente + verso Econômica concluídos; Premium, Pisos e Quadras e Esmalte em produção (prompts prontos)
- Seção "Obras Atendidas" no site com fotos reais de canteiro
- Abordagem direta das 100 construtoras mapeadas (WhatsApp + Brevo)
- Formulário do site conectado ao Brevo via API (código pronto, teste end-to-end pendente)

### Próximos passos (ordem de execução)
1. Testar integração formulário → Brevo (precisa da API key do Brevo)
2. Lançar campanhas Meta Ads (MEP Massas primeiro, depois MEP COLOR)
3. Google Ads — busca por palavras-chave do setor
4. Calendário editorial Instagram — próximos 30 dias
5. Abordar lista de 100 construtoras (WhatsApp direto + sequência Brevo)

---

## Produto — informação corrigida

- A embalagem "bisnaga" MEP Massas é um **saco aplicador triangular** (tipo manga de confeitar / piping bag), não frasco squeezable. Corte na ponta para aplicar. Disponível em 1kg e 3kg.
- Concorrente de referência para o rótulo: **Argapoli** (saco triangular com muita informação visual — diferenciais, rendimento, ícones)
